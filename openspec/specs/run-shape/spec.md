# run-shape Specification

## Purpose
How wide a run gets and how many times its work repeats — decided by what the
run finds rather than by what the author of the workflow could foresee.

A `dag` node's tasks are a list written before anyone has seen the data. These
requirements cover the two ways that list stops being the whole story: a task
that becomes one task per item of an upstream result, and a task that keeps
running while it keeps finding. Both are applied by code. A worker's whole say
is the list it reports; the count, the cap, the deduplication and the stop
condition are the runtime's.
