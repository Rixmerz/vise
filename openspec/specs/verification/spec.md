...
## Purpose
How a worker's claim becomes the runtime's conclusion.

`TaskResult.verdict` is what a worker says about its own work; `SUCCEEDED` is
what the runtime concludes after an agent that did not do the work agrees.
These requirements cover how many such agents a task may require and how their
answers are folded into one.
